...
def read_single_tfrecord(tfrecord_file, batch_size, net):
    ...
    if net == 'net1':
        image_size = 12
    elif net == 'net2' or net == 'net2_v2':
        image_size = 24
    elif net == 'net3' or net == 'net3_v2':
        image_size = 48
    else:
...
def read_single_tfrecord_for_net1_2(tfrecord_file, batch_size, net):
    # generate a input queue
    # each epoch shuffle
    filename_queue = tf.train.string_input_producer([tfrecord_file],shuffle=True)
    # read tfrecord
...
def read_multi_tfrecords(tfrecord_files, batch_sizes, net):
    pos_dir, landmark_dir = tfrecord_files
    pos_batch_size, landmark_batch_size = batch_sizes
    ...
    images = tf.concat([pos_image, landmark_image], 0, name="concat/image")
    labels = tf.concat([pos_label, landmark_label], 0, name="concat/label")
    rois = tf.concat([pos_roi, landmark_roi], 0, name="concat/roi")
    landmarks = tf.concat([pos_landmark, landmark_landmark], 0, name="concat/landmark")
    return images, labels, rois, landmarks


def read_multi_tfrecords_v2(tfrecord_files, batch_sizes, net):
