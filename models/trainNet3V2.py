...
import sys
import argparse
from toolsFunc.tfrecordReaderUtils import read_single_tfrecord
from models.configs import config
from models.nets import Net_1, Net_2, Net_3, Net_3_V2
import cv2
...
def train(netFactory, modelPrefix, endEpoch, dataPath, display=100, baseLr=0.01, gpus=""):
    print("Now start to train...stage: net3_v2")
    net = "net3_v2"

    pos_dir = os.path.join(dataPath, 'pos.tfrecord')
    image_batch, label_batch, bbox_batch, landmark_batch = read_single_tfrecord(pos_dir, config.BATCH_SIZE, net)
    total_num = sum(1 for _ in tf.python_io.tf_record_iterator(pos_dir))
    # ratio
    ratio_bbox_loss, ratio_landmark_loss = 1.0, 1.5
    image_size = 48
    # define placeholder
    ...
    # class,regression
    bbox_loss_op, landmark_loss_op, L2_loss_op = netFactory(input_image, label, bbox_target,
                                                                                landmark_target, training=True)

    # train,update learning rate(3 loss)
    train_op, lr_op = train_model(baseLr,
                                ratio_bbox_loss * bbox_loss_op + ratio_landmark_loss * landmark_loss_op + L2_loss_op,
                                  8000)
    # init
  ...
  #  model_path = 'D:/alex/CascadeNetwork3/tmp/model/net3_v2'
    ...
    try:
        for step in range(MAX_STEP):
            ...
            if (step + 1) % display == 0:
                ...
                bbox_loss, landmark_loss, L2_loss, lr = sess.run(
                    [bbox_loss_op, landmark_loss_op, L2_loss_op, lr_op],
                ...
                print(
                    "%s [%s] Step: %d, bbox loss: %4f, landmark loss: %4f,L2 loss: %4f,lr:%f " % (
                        datetime.now(), net, step + 1, bbox_loss, landmark_loss, L2_loss, lr))
            # save every two epochs
            if i * config.BATCH_SIZE > total_num * 100:
                epoch = epoch + 1
                i = 0
                saver.save(sess, modelPrefix, global_step=epoch * 100)
...
if __name__ == "__main__":
    dataPath = os.path.join(rootPath, "tmp/data/%s" % ("net3_v2"))
    modelPrefix = os.path.join(rootPath, "tmp/model/%s/%s" % ("net3_v2", "net3_v2"))
    ...
    train(Net_3_V2, modelPrefix, epoch, dataPath, display=display_steps, baseLr=lr, gpus='0')
