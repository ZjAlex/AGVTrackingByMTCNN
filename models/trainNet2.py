...
def train(netFactory, modelPrefix, endEpoch, dataPath, display=200, baseLr=0.01, gpus=""):
    print("Now start to train stage: net2")
    net = 'net2'
    # set GPU
    if gpus:
    ...
    # ratio
    image_size = 24
    ratio_cls_loss, ratio_bbox_loss, ratio_landmark_loss = 1.0, 1.0, 0.5
    # define placeholder
 ...
 #   model_path = 'D:/alex/tmp/model/net2/net2'
...
if __name__ == "__main__":
    dataPath = os.path.join(rootPath, "tmp/data/%s" % ("net2"))
    modelPrefix = os.path.join(rootPath, "tmp/model/%s/%s" % ("net2", "net2"))
    ...
    train(Net_2, modelPrefix, epoch, dataPath, display=display_steps, baseLr=lr, gpus='0')
